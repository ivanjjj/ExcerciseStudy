...
correlated_variables <- findCorrelation(cor(training[,4:56]), cutoff = 0.6, exact = TRUE)

training_2 <- training[,-(correlated_variables+4)]

fit_rpart <- train(classe~., method="rpart", data=training_2)

fit_gbm <- train(classe~., method="gbm", data=training_2)
fit_lda <- train(classe~., method="lda", data=training_2)
fit_rf <- train(classe~., method="rf", data=training_2)
